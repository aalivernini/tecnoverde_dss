...
<style>
    ...
    #myTable th.asc .arrow:before {
        content: '\f0b2';
    }

    #myTable th.desc .arrow:before {
        content: '\f0b3';
    }

    #myTable th:hover .arrow {
        opacity: 1;
    }
    ...
    .login::after {
        left: 35%;
        bottom: -125%;
        background-color: rgb(122 182 52);
...
<body id="body">
                ...
                <section class="primary" id="portfolio">
                        ...
                        <div class="row">
                            ...
                            <form id="myForm" style="width: 100%; text-align: center;" method="post" action="" enctype="multipart/form-data">
                                ...
                                <div class="other_select">
                                    ...
                                    <div class="search-item">
                                        <select class="select" style="text-transform: uppercase;" id="esigenza_suolo" name="esigenza_suolo" list="esigenza_suolo" autocomplete="off">
                                            <datalist id="esigenza_suolo-dl">
                                                <option style="color: gray;" selected value="">Esigenza Suolo</option>
                                                <?php
                                    ...
                                    <div class="search-item">
                                        <select class="select" style="text-transform: uppercase;" id="dimensione_foglie" name="dimensione_foglie" list="dimensione_foglie" autocomplete="off">
                                            <datalist id="dimensione_foglie-dl">
                                                <option style="color: gray;" selected value="">Dimensione foglia</option>
                                                <?php
                                                $sql_cat = "SELECT column_name FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME = 'dimensione_foglie' AND COLUMN_NAME != 'id' AND COLUMN_NAME != 'id_specie' AND COLUMN_NAME != 'created' AND COLUMN_NAME != 'boa_id'  AND COLUMN_NAME != 'value';";
                                                $result_cat = mysqli_query($conn, $sql_cat);
                                                while ($cat_row = mysqli_fetch_array($result_cat)) {
                                                ?>
                                                    <option style="text-transform: uppercase;" value="<?php echo $cat_row['column_name']; ?>"><?php echo $cat_row['column_name']; ?></option>
                                        ...
                                        <span class="icon-info">
                                            <i class="fas fa-info-circle"></i>
                                            <span class="info-tooltip">La dimensione di una foglia di una pianta si riferisce alla misura della superficie fogliare, ovvero alla grandezza e alla forma della foglia stessa. La dimensione delle foglie è influenzata dalla genetica della pianta, dal clima, dalle condizioni ambientali e dal tipo di suolo in cui la pianta cresce. La conoscenza della dimensione delle foglie può essere utile per la scelta della specie e delle tecniche di coltivazione, oltre che per la sua gestione e valorizzazione.</span>
                                        </span>
                                    </div>
                                ...
                                <div style="width: 100%;" class="flexato" id="box-btn">
                                    ...
                                    <div>
                                        ...
                                        <!-- <input class="btn-cerca" id="btn-spara" style="background-color: red;" type="button" value="spara"> -->
                                        <!-- <input class="btn-cerca" id="btn-piu" style="background-color: red;" type="button" value="piu"> -->
                    ...
                    <div class="footer mt-5 pt-3" style="margin-bottom: -88px;">
                        <footer class="d-flex p-4 " style="justify-content:space-evenly;"> <span>All Rights Reserved by
                                <a class="footer-link" href="https://mvc-technology.eu">mvc-technology S.p.A.</a></span>
    ...
    <script>
        $(document).ready(function() {
            ...
            async function fetchJsonRecursive() {
                try {
                    const response = await fetch("includes/json.php");
                    ...
                    if (piante.length > 0) {
                        // Chiamata ricorsiva se ci sono ancora elementi nel JSON
                        await fetchJsonRecursive();
                    } else {
                        // JSON completo ricevuto, esegui altre operazioni
                        console.log("JSON completo:", arrayIntero);
                        // Chiamare altre funzioni qui una volta completato il JSON
                    }
                } catch (error) {
            ...
            // Inizializza l'arrayIntero
            var arrayIntero = [];

            // Avvia il fetch del JSON
            fetchJsonRecursive();
            ...
            var stessaSelectProvvisorio = {
                selezione: [],
                idSelezione: []
            };


            function filtraArray(arrayIntero, valoriSelezionati) {
                // Crea un nuovo array per i risultati filtrati
                const arrayFiltrato = [];

                // Cicla attraverso gli oggetti dell'arrayIntero
                for (const oggetto of arrayIntero) {
                    let match = true;

                    // Cicla attraverso i valori selezionati
                    for (const valore of valoriSelezionati) {
                        const select = valore.select;
                        const filtro = valore.filtro;

                        // Verifica se l'oggetto soddisfa il filtro
                        if ((oggetto[select] === 1) !== filtro) {
                            match = false;
                            break;
                        }
                    }

                    // Se tutti i filtri corrispondono, aggiungi l'oggetto all'array filtrato
                    if (match) {
                        arrayFiltrato.push(oggetto);
                    }
                }
            ...
            $('select').change(function() {
                ...
                for (var i = 0; i < valoriSelezionati.length; i++) {
                    ...
                    listaValori += '<li value="' + selectId + '" style="margin-left:7px">' + valoriSelezionati[i] + '<span  class="selezionato" value="' + valoriSelezionati[i] + '"> x</span></li>';
                ...
                // -----------------container test
                const risultato = filtraArray(arrayIntero, valoriSelezionati);
                console.log(risultato); // // Ottieni tutte le select con il tag select
                // var selects = document.querySelectorAll('select');
                // // Itera su tutte le select
                // for (var i = 0; i < selects.length; i++) {
                //     var select = selects[i];
                //     // Ottieni l'ID della select corrente
                //     var selectId = select.id;

                //     // Aggiungi l'ID della select corrente all'array
                //     selectedIds.push(selectId);
                // }

                // var diversaSelectProvvisorio = [];

                // for (let i = 0; i < valoriSelezionati.length; i++) {
                //     const element = valoriSelezionati[i];

                //     // Seleziona l'elemento option
                //     var optionElement = document.querySelector('option[value="' + element + '"]');

                //     // Verifica se l'elemento option esiste e ha un elemento padre
                //     if (optionElement && optionElement.parentElement) {
                //         // Risali all'elemento padre, che dovrebbe essere la select
                //         var selectElement = optionElement.parentElement.closest('select');

                //         // Verifica se l'elemento select esiste e ha un ID definito
                //         if (selectElement && selectElement.id) {
                //             var selectId = selectElement.id;

                //             getSelectIndex = selectedIds.indexOf(selectId);
                //             // console.log(getSelectIndex);

                //             if ((!diversaSelectProvvisorio[getSelectIndex] || diversaSelectProvvisorio[getSelectIndex] == 'undefined') && !diversaSelectProvvisorio.includes(element)) {
                //                 diversaSelectProvvisorio[getSelectIndex] = element;



                //             } else if (!stessaSelectProvvisorio.selezione.includes(element)) {

                //                 stessaSelectProvvisorio.selezione.push(element);
                //                 stessaSelectProvvisorio.idSelezione.push(selectId);
                //             }
                //         }
                //     }
                // }

                // console.log("stessa " + stessaSelectProvvisorio.selezione);
                // console.log("diversa " + diversaSelectProvvisorio);


                // // selectedIds -> array di tutti gli id di tutte le select

                // //diversaSelectProvvisorio -> array di tutti i filtri selezionati

                // //stessaSelectProvvisorio -> array di appoggio dove vengono puhati i valori dove l'id della selct corrisponte risulta   gia occupato


                // if (stessaSelectProvvisorio.selezione.length > diversaSelectProvvisorio.length) {

                //     for (let i = 0; i < stessaSelectProvvisorio.selezione.length; i++) {

                //         const element = stessaSelectProvvisorio.selezione[i];

                //     }

                // } else {
                //     for (let i = 0; i < diversaSelectProvvisorio.length; i++) {

                //         const element = diversaSelectProvvisorio[i];
            ...
            $('body').on('click', '.selezionato', function() {
                ...
                if (index !== -1) {
                    ...
                    objSelectId.arrSelectId.splice(index, 1)
                    objSelectId.arrOptionValue.splice(index, 1)
                }


                console.log(objSelectId)
                $(this).parent().remove();

                // --------------------------
                $('#' + valoreSelezionato).val('').trigger('change');
                oggettiFiltrati = arrayIntero.filter(function(oggetto) {
                    // const selectId = this.value
                    let selectId = objSelectId.arrSelectId.indexOf(oggetto)
                    let optionValue = objSelectId.arrSelectId.indexOf(oggetto)

                    const selezioniStessaSelect = valoriSelezionati.filter(function(selezione) {
                        return $('#' + objSelectId.arrSelectId[valoriSelezionati.indexOf(selezione)]).find('option[value="' + objSelectId.arrOptionValue[valoriSelezionati.indexOf(selezione)] + '"]').length > 0;
                    });

                    const selezioniAltreSelect = valoriSelezionati.filter(function(selezione) {
                        return $('#' + objSelectId.arrSelectId[valoriSelezionati.indexOf(selezione)]).find('option[value="' + objSelectId.arrOptionValue[valoriSelezionati.indexOf(selezione)] + '"]').length === 0;
                    });


                    if (selezioniStessaSelect.length > 0) {
                        return selezioniStessaSelect.some(function(selezione) {
                            return oggetto[selezione] === "1";
                        });
                    } else {
                        return selezioniAltreSelect.every(function(selezione) {
                            return oggetto[selezione] === "1";
                        });
                    }
                });
                ...
                // Aggiorna la select di appartenenza
                // $('#' + selectId).val('');
            });
